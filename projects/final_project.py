...
current_room = 0
instructions = ["When presented a room choice or an interaction choice input the number of choice you want to select", "When choosing something from your inventory please put in the full name, sorry about this I know it's inconvenient"]
...
def player_turn(player_attack, player_specialatk, enemy_defense, enemy_specialdef, player_inventory):
    ...
    while True:
        if action == "1":
            ...
            source = "damage"
            return damage, source
        elif action == "4" and len(player_inventory) > 0:
...
def enemy_turn(player_specialdef, player_defense, enemy_attack, magic):
    source = ""
...
def battle(player_speed, player_health, enemy_speed, enemy_health, enemy):
    ...
    if player_speed > enemy_speed:
            while True:
                condition, source = player_turn(stats["player"]["attack"], stats["player"]["special attack"], stats[enemy]["defense"], stats[enemy]["special_defense"], inventory)
                if source == "damage":
                ...
                condition, source = enemy_turn(stats["player"]["special defense"], stats["player"]["defense"], stats[enemy]["attack"], stats[enemy]["special attack"], stats[enemy]["magic"])
                if source == "damage":
                    player_health -= condition
                elif source == "heal":
                    enemy_health += condition
            ...
            while True:
                condition, source = enemy_turn(stats["player"]["special defense"], stats["player"]["defense"], stats[enemy]["attack"], stats[enemy]["magic"])
                if source == "damage":
                    player_health -= condition
                elif source == "heal":
                    enemy_health += condition
                ...
                condition, source = player_turn(stats["player"]["attack"], stats["player"]["special attack"], stats[enemy]["defense"], stats[enemy]["special defense"], inventory)
                if source == "damage":
                    enemy_health -= condition
                elif source == "heal":
                    player_health += condition
...
def dons_office(player_name):
    ...
    while True:
        if press == "1":
            ...
            winner = battle(stats["player"]["speed"], stats["player"]["health"], stats["Don"]["speed"], stats["Don"]["health"], "Don")
            break
        elif press == "2":
            print("A booming voice from under the room says 'PRESS IT PLEASE PLEASE PLEASE DON'T DENY ME THIS")
        else:
            print("That's not even an option.")
            press = input("To press (1) or not to press (2)?")
    if winner == "player":
...
def front_room():
    print("This room is a front, no matter what those wizards can't be trusted.")
...
def meeting_room(grunts):
    ...
    if grunts == True:
        ...
        winner = battle(stats["player"]["speed"], stats["player"]["health"], stats["grunt"]["speed"], stats["grunt"]["health"], "grunt")
        if winner == "player":
...
while play_or_quit == "1":
    ...
    time.sleep(5)
    print("Well it's not so much me as my father sir... He's under contract with the mafia, and I'm afriad they have no good intentions with him. The dark lord has got him under a terrible spell... Oh I just don't know what to Do! Please say you'll help me!")
    time.sleep(4)
    print("The... Dark lord? YOu mean the Don? The Godfather?")
    time.sleep(4)
    print("Uhhh no, I mean the dark lord Abracadelio, the wizard... He was on the news today?")
    time.sleep(4)
    print("...What?")
    time.sleep(4)
    print("Never mind that! Just say you'll help my father oh please!")
    time.sleep(4)
    print("Jeez fine.")
    time.sleep(4)
    print("...And that, is how this terrible sorrowful tale began...")

    print("But finally, you made it... You've tracked down the base of those idiots and finllaly you're going to destroy them once and for all. Suprisingly though, from the outside it looks like a very normal resturaunt. Once you enter it's the same. It's a homely place, with tables and even an olive bar. Looks like these guys do have some semblance of class.")
